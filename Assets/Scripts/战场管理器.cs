...
public class 战场管理器 : MonoBehaviour
{
    ...
    public void 启动战场倒计时(国家信息库 国家, DateTime? 开始时间 = null)
    {
        ...
        // 如果剩余时间已经<=15秒，立即显示战场入口UI
        if (剩余时间 <= 15 && !已显示战场入口UI)
        {
            Debug.Log($"[战场管理器] 启动倒计时时剩余时间 {剩余时间} <= 15，立即显示战场入口UI");
            已显示战场入口UI = true;
            显示战场入口UI();
        }
    ...
    public void 同步服务端倒计时(国家信息库 国家, int 剩余秒数)
    {
        ...
        if (国家 == null)
        {
            Debug.LogWarning("[战场管理器] 同步倒计时失败：国家信息为空");
            return;
        }

        if (国家.宣战家族1 == null || 国家.宣战家族2 == null)
        {
            Debug.LogWarning($"[战场管理器] 同步倒计时失败：宣战家族信息不完整。家族1={国家.宣战家族1?.家族名字 ?? "null"}, 家族2={国家.宣战家族2?.家族名字 ?? "null"}");
            return;
        }
        ...
        if (!是否正在倒计时)
        {
            Debug.Log($"[战场管理器] 启动战场倒计时: {国家.宣战家族1.家族名字} VS {国家.宣战家族2.家族名字}，剩余 {剩余秒数} 秒");
        ...
        else
        {
            ...
            // 输出倒计时信息（每5秒或最后10秒输出，减少日志量但保持关键信息可见）
            // 注意：倒计时协程中也会输出，这里作为服务端同步的确认
            if (剩余秒数 % 5 == 0 || 剩余秒数 <= 10)
            {
                Debug.Log($"[战场管理器] 同步服务端倒计时: 剩余 {剩余秒数} 秒");
            }
        }

        // 检查是否需要显示战场入口UI（剩余秒数<=15时）
        if (剩余秒数 <= 15 && !已显示战场入口UI)
        {
            Debug.Log($"[战场管理器] 剩余秒数 {剩余秒数} <= 15，立即显示战场入口UI");
            已显示战场入口UI = true;
            显示战场入口UI();
        }
    }
    ...
    private IEnumerator 倒计时协程()
    {
        while (剩余时间 > 0f && 是否正在倒计时)
        {
            ...
            // 输出倒计时信息（每5秒或最后10秒输出，与服务端同步保持一致）
            if (整数秒数 % 5 == 0 || 整数秒数 <= 10)
            {
                Debug.Log($"[战场管理器] 战场倒计时: {整数秒数}秒");
            }
        }
